...
import transML.TestsLang.TestCase;
import transML.TestsLang.TestModel;
import transML.TestsLang.TestSuite;
import transML.TestsLang.TestsLangFactory;
...
public class WitnessGenerator extends AbstractHandler {
	...
	private transMLMessageConsole console = new transMLMessageConsole();
...
//		String sTargetMM       = fullpath+"SPL.ecore";
		...
		try {
			// load meta-models
			List<EPackage> errorMMs = MetaModel.loadEcoreMetamodel(sErrorMM);
			...
			if (ocl_constraints.size() != errorMMs.size()) 
				throw new WitnessException("The error meta-model contains " + errorMMs.size() + " errors, but there are " + ocl_constraints.size() + " ocl constraints.");
...
if (completeClass!=null) {				
	 ...
	 */
	private List<String> loadOclConstraints (String file) throws WitnessException {
		...
		catch (FileNotFoundException e) { e.printStackTrace(); throw new WitnessException("File not found: " + file, e); }
        catch (IOException e) { throw new WitnessException(e); } 
		finally {
			try { if (br!=null) br.close(); } 
			catch (IOException ex) { throw new WitnessException(ex); }
	    }
	 ...
	 * @throws transException
	 */
	protected String generateWitness (String path, EPackage metamodel, String ocl_constraint, int index) throws WitnessException {
